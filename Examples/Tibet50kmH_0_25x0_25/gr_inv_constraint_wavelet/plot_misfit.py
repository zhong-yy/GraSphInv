...
def plot_lambda_misfit(filename_iteration,filename_lambda,outname):
    ...
    for i in range(0,int(iteration_number)):
        ...
        plt.loglog(lam[start:end],misfit[start:end],label='Iteration '+str(i+1),color='black',linestyle='-',linewidth=1,marker=mk[i],markersize=mks[i])
...
plot_lambda_misfit('Iteration_misfit_GN','lambda_misfit_GN','lambda_misfit_tibet.eps')
