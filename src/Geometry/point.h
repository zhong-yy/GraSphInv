...
inline Point::Point(const Point &p) : TOL(1.0e-10) {
    for (unsigned int i = 0; i < 3; i++) _coords[i] = p._coords[i];
}
...
inline bool Point::operator==(const Point &rhs) const {
    // bool temp = (std::abs(_coords[0] - rhs._coords[0]) < TOL) &&
    //            (std::abs(_coords[1] - rhs._coords[1]) < TOL) &&
    //            (std::abs(_coords[2] - rhs._coords[2]) < TOL);
    // return temp;
