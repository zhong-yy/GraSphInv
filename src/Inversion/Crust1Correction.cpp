...
Crust1Correction::Crust1Correction() {
  this->reference_surface = 0;        // initialization
  this->reference_crust_density = 0;  // initialization
  this->reference_mantle_density = 0;
  this->path_to_crust1_0 = "";
  this->n_fields = 0;
  this->field_flag = 0;
...
void Crust1Correction::show_crust_info() {
  ...
  // reference_upper_crystalline_crust = average_upper_crystalline;
  // reference_middle_crystalline_crust = average_middle_crystalline;
  // reference_lower_crystalline_crust = average_lower_crystalline;
    ...
    for (int j = 0; j < 360; j++) {
      ...
      if (abs(crust1_bnds[i][j][5] - crust1_bnds[i][j][6]) > 1e-5) {
        ...
        crystalline_crust.cells[0][id]->set_parameter(
            crust1_rho[i][j][5] * 1000 - reference_crust_density);
        id++;
      }
      ...
      if (abs(crust1_bnds[i][j][6] - crust1_bnds[i][j][7]) > 1e-5) {
        ...
        crystalline_crust.cells[0].push_back(
            new Cell(r0, theta0, phi0, r1, theta1, phi1, 0, 1, true));
        crystalline_crust.cells[0][id]->set_parameter(
            crust1_rho[i][j][6] * 1000 - reference_crust_density);
        id++;
      }
      ...
      if (abs(crust1_bnds[i][j][7] - crust1_bnds[i][j][8]) > 1e-5) {
        ...
        crystalline_crust.cells[0].push_back(
            new Cell(r0, theta0, phi0, r1, theta1, phi1, 0, 1, true));
        crystalline_crust.cells[0][id]->set_parameter(
            crust1_rho[i][j][7] * 1000 - reference_crust_density);
        id++;
      }
    }
  }
...
void Crust1Correction::build_mesh_of_moho() {
  ...
  int id = 0;
  double r_crust = reference_surface - 40000;
  ...
  for (int i = 0; i < 180; i++) {
    for (int j = 0; j < 360; j++) {
      ...
      if (abs(r_moho - r_crust) < 1e-5) {
        // moho is coincide with the reference crust-mantle boundary
        // do nothing
      } else if (r_moho > r_crust) {
        moho.cells[0].push_back(
            new Cell(r_crust, theta0, phi0, r_moho, theta1, phi1, 0, 1, true));
        moho.cells[0][id]->set_parameter(reference_mantle_density -
                                         reference_crust_density);
        id++;
      } else {
        // moho is below the reference crust-mantle boundary
        moho.cells[0].push_back(
            new Cell(r_moho, theta0, phi0, r_crust, theta1, phi1, 0, 1, true));
        moho.cells[0][id]->set_parameter(reference_crust_density -
                                         reference_mantle_density);
        id++;
      }
    }
  }
...
void Crust1Correction::read_configuration_file(string file_name) {
  ...
  iss.str("");
  iss.str(line);
  iss >> reference_crust_density;
  cout << reference_crust_density << endl;

  next_valid_line(ifs, line);
  iss.clear();
  iss.str("");
  iss.str(line);
  iss >> reference_mantle_density;
  cout << reference_mantle_density << endl;
