...
Crust1Correction2::Crust1Correction2() {
...
void Crust1Correction2::read_crust_data() {
...
void Crust1Correction2::show_crust_info() {
...
void Crust1Correction2::build_mesh_of_sediments() {
    ...
    for (int j = 0; j < 360; j++) {
      ...
      if (abs(crust1_bnds[i][j][2] - crust1_bnds[i][j][3]) > 1e-5) {
        ...
        if (r0 > r_upper_crust || abs(r0 - r_upper_crust) < 1e-5) {
          // within upper crust
          sediments.cells[0].push_back(
              new Cell(r0, theta0, phi0, r1, theta1, phi1, 0, 1, true));
          sediments.cells[0][id]->set_parameter(crust1_rho[i][j][2] * 1000 -
                                                reference_upper_crust);
          id++;
        } else if (r1 < r_upper_crust || abs(r1 - r_upper_crust) < 1e-5) {
          // within lower crust
          sediments.cells[0].push_back(
              new Cell(r0, theta0, phi0, r1, theta1, phi1, 0, 1, true));
          sediments.cells[0][id]->set_parameter(crust1_rho[i][j][2] * 1000 -
          ...
          sediments.cells[0].push_back(new Cell(r0, theta0, phi0, r_upper_crust,
                                                theta1, phi1, 0, 1, true));
          sediments.cells[0][id]->set_parameter(crust1_rho[i][j][2] * 1000 -
                                                reference_lower_crust);
          id++;

          sediments.cells[0].push_back(new Cell(r_upper_crust, theta0, phi0, r1,
                                                theta1, phi1, 0, 1, true));
          sediments.cells[0][id]->set_parameter(crust1_rho[i][j][2] * 1000 -
                                                reference_upper_crust);
          id++;
      ...
      if (abs(crust1_bnds[i][j][3] - crust1_bnds[i][j][4]) > 1e-5) {
        ...
        if (r0 > r_upper_crust || abs(r0 - r_upper_crust) < 1e-5) {
          ...
          sediments.cells[0].push_back(
              new Cell(r0, theta0, phi0, r1, theta1, phi1, 0, 1, true));
          sediments.cells[0][id]->set_parameter(crust1_rho[i][j][3] * 1000 -
                                                reference_upper_crust);
          id++;
        } else if (r1 < r_upper_crust || abs(r1 - r_upper_crust) < 1e-5) {
          // within lower crust
      ...
      if (abs(crust1_bnds[i][j][4] - crust1_bnds[i][j][5]) > 1e-5) {
        ...
        if (r0 > r_upper_crust || abs(r0 - r_upper_crust) < 1e-5) {
          ...
          sediments.cells[0].push_back(
              new Cell(r0, theta0, phi0, r1, theta1, phi1, 0, 1, true));
          sediments.cells[0][id]->set_parameter(crust1_rho[i][j][4] * 1000 -
                                                reference_upper_crust);
          id++;
        } else if (r1 < r_upper_crust || abs(r1 - r_upper_crust) < 1e-5) {
          // within lower crust
...
void Crust1Correction2::build_mesh_of_crystalline_crust() {
      ...
      if (abs(crust1_bnds[i][j][5] - crust1_bnds[i][j][6]) > 1e-5) {
        ...
        if (r0 > r_upper_crust || abs(r0 - r_upper_crust) < 1e-5) {
          ...
          crystalline_crust.cells[0].push_back(new Cell(
              r0, theta0, phi0, r_upper_crust, theta1, phi1, 0, 1, true));
          crystalline_crust.cells[0][id]->set_parameter(
              crust1_rho[i][j][5] * 1000 - reference_lower_crust);
          id++;

          crystalline_crust.cells[0].push_back(new Cell(
              r_upper_crust, theta0, phi0, r1, theta1, phi1, 0, 1, true));
          crystalline_crust.cells[0][id]->set_parameter(
              crust1_rho[i][j][5] * 1000 - reference_upper_crust);
          id++;
        }
...
void Crust1Correction2::build_mesh_of_moho() {
    ...
    for (int j = 0; j < 360; j++) {
      ...
      if (abs(r_moho - r_lower_crust) < 1e-5) {
        ...
        if (r_moho > r_upper_crust) {
          ...
          moho.cells[0].push_back(new Cell(r_lower_crust, theta0, phi0,
                                           r_upper_crust, theta1, phi1, 0, 1,
                                           true));
          moho.cells[0][id]->set_parameter(reference_mantle -
                                           reference_lower_crust);
          id++;

          moho.cells[0].push_back(new Cell(r_upper_crust, theta0, phi0, r_moho,
                                           theta1, phi1, 0, 1, true));
          moho.cells[0][id]->set_parameter(reference_mantle -
                                           reference_upper_crust);
          id++;
        } else {
          moho.cells[0].push_back(new Cell(r_lower_crust, theta0, phi0, r_moho,
                                           theta1, phi1, 0, 1, true));
          moho.cells[0][id]->set_parameter(reference_mantle -
                                           reference_lower_crust);
          id++;
        ...
        moho.cells[0].push_back(new Cell(r_moho, theta0, phi0, r_lower_crust,
                                         theta1, phi1, 0, 1, true));
...
void Crust1Correction2::read_data_to_be_corrected() {
...
void Crust1Correction2::out_file() {
  ...
  for (int j = 0; j < n_com; j++) {
    ...
    for (int i = 0; i < n_ob; i++) {
      // get_n_obs() obtains number of observation points, data size = n_com*
      const Point& p = ob(i);
      out_s << fixed;
  ...
  for (int j = 0; j < n_com; j++) {
    ...
    for (int i = 0; i < n_ob; i++) {
      ...
      out_s << setw(25) << setprecision(7) << left
            ...
            << p(2) * 180.0 / GS::PI - 180.0;
      out_s << scientific;
      out_s << setw(30) << setprecision(15) << left << gra_moho(i + j * n_ob)
            << endl;
    }
  }
}
void Crust1Correction2::read_configuration_file(string file_name) {
  ...
  cout << "Reference level is " << setprecision(7) << reference_surface << endl;
...
void Crust1Correction2::line_process(std::string& line,
...
istream& Crust1Correction2::next_valid_line(istream& input_stream,
                                            string& line) {
  while (std::getline(input_stream, line)) {
    line_process(line);
    if (line.empty()) {
...
void Crust1Correction2::gravity_effects_of_sediments() {
...
void Crust1Correction2::gravity_effects_of_crystalline_crusts() {
  Timer t;
  t.start();
...
void Crust1Correction2::gravity_effects_of_moho() {
  Timer t;
  t.start();
