#include "GN_Inversion_Wavelet.h"
void GN_Inversion_Wavelet::invert() {
...
void GN_Inversion_Wavelet::set_CG_parameter(double cg_tol_,
...
void GN_Inversion_Wavelet::display_inversion_parameters() const {
    display_info_fields();
    cout << endl;
...
VectorXd GN_Inversion_Wavelet::solve_cg(
    ...
    r += P * GTwds;
    r += sqrt(lambda) * P * Ws.transpose() * s.segment(Nd, Nm);
    r += sqrt(lambda) * P * Wr.transpose() * s.segment(Nd + Nm, Nm);
    r += sqrt(lambda) * P * Wtheta.transpose() * s.segment(Nd + 2 * Nm, Nm);
    r += sqrt(lambda) * P * Wphi.transpose() * s.segment(Nd + 3 * Nm, Nm);
    if (use_cross_gradient_constraint == true) {
        r += P * Tr.transpose() * s.segment(Nd + 4 * Nm, Nm);
        r += P * Ttheta.transpose() * s.segment(Nd + 5 * Nm, Nm);
        r += P * Tphi.transpose() * s.segment(Nd + 6 * Nm, Nm);
    }
    double beta = 0.0;
    VectorXd p(Nm);
    p.setZero();
...
// void GN_Inversion_Wavelet::get_curvature(const vector<double>& x,
